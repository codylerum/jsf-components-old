...
function startZclip(id, zclip_path, copyClass, copiedClass) {
        if (zclip == null) {
    		ZeroClipboard.config({ swfPath : zclip_path, cacheBust: false });
            zclip = new ZeroClipboard();
            zclip.on('beforecopy', function(zclip, args) {
            	var text = $(zclip.target).data('clipboard-text')
            	zclip.setText(text);
            });
            zclip.on('aftercopy', function(zclip, args) {
                    $('.clipboard-copy-div.' + copiedClass).removeClass(copiedClass).addClass(copyClass);
                    $(zclip.target).removeClass(copyClass).addClass(copiedClass);
            });
            zclip.on('error', function(zclip, args) {
        ...
        zclip.clip(document.getElementById(id));
}